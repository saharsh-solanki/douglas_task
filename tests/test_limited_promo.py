...
def scraper():
    ...
    scraper.goto_parfum()
    scraper.select_limited_promo_filter()
    yield scraper
    scraper.destroy()


class TestLimitedPromo:
    @pytest.mark.parametrize(
        "brand, product_type, for_whom, gift",
        [
            ("4711", None, None, None),
            (None, None, "Unisex", None),
            # ("Chanel", "Eau de Parfum", "Women", "Weihnac   hten")
        ],
    )
    def test_limited_promo_filter(self, scraper, brand, product_type, for_whom, gift):
        if brand:
        ...
        save_to_excel("Limited", brand, product_type, for_whom, gift, count)
